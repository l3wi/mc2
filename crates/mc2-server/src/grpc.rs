//! Agent gRPC service (`mc2.agent.v1.AgentService`).
...
use mc2_api::agent::agent_service_server::AgentService;
use mc2_api::agent::{
...
use mc2_store::{hash_token, NodeHeartbeat, NodeJoin, SecretsKey, Store};
use rand::RngCore;
use std::sync::Arc;
...
fn gen_node_token() -> String {
    ...
    format!("mc2nt_{}", hex::encode(bytes))
}
...
impl AgentService for AgentSvc {
    ...
    async fn heartbeat(
            ...
            .await
        {
            ...
            Err(mc2_store::StoreError::Unauthorized) => {
                Err(Status::unauthenticated("invalid node token"))
            }
            Err(mc2_store::StoreError::NotFound(_)) => Err(Status::not_found("unknown node")),
            Err(e) => {
    ...
    async fn sync(&self, request: Request<SyncRequest>) -> Result<Response<SyncResponse>, Status> {
        ...
        if !mc2_store::verify_token(&req.node_token, &node.node_token_hash) {
        ...
        for i in rows {
            ...
            instances.push(mc2_api::agent::DesiredInstance {
                instance_id: i.id,
                stack: i.stack,
                service: i.service,
                ordinal: i.ordinal,
